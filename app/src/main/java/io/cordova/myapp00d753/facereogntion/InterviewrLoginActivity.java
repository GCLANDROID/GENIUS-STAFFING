...
public class InterviewrLoginActivity extends AppCompatActivity implements View.OnClickListener {
  ...
  private void initView(){
      pref=new Pref(InterviewrLoginActivity.this);
      ...
      mAppUpdateManager.getAppUpdateInfo().addOnSuccessListener(new OnSuccessListener<AppUpdateInfo>() {
          ...
          public void onSuccess(AppUpdateInfo appUpdateInfo) {
              if (appUpdateInfo.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE && appUpdateInfo.isUpdateTypeAllowed(AppUpdateType.IMMEDIATE)){
                  try {
                      mAppUpdateManager.startUpdateFlowForResult(appUpdateInfo, AppUpdateType.IMMEDIATE,InterviewrLoginActivity.this,RC_APP_UPDATE);
        ...
        if (view==binding.llSignIn){
           if (binding.etUserId.getText().toString().length()>0){
               if (binding.etPassword.getText().toString().length()>0){
                   Intent intent=new Intent(InterviewrLoginActivity.this,InterViewerPanelActivity.class);
                   startActivity(intent);
                   finish();
               }else {
                   Toast.makeText(InterviewrLoginActivity.this,"Please enter Password",Toast.LENGTH_LONG).show();
               }

           }else {
               Toast.makeText(InterviewrLoginActivity.this,"Please enter User ID",Toast.LENGTH_LONG).show();
           }
        }
